...
export default function DashboardScreen() {
  ...
  useKeepAwake(isShiftActive ? "active-shift" : undefined);
  ...
  // Calculate countdown timer — must be before the mandatory break useEffect
  const driverType = (user as any)?.driverType ?? "small_passenger";
  const drivingLimitSeconds = getDrivingLimitSeconds(driverType);
  // FIX: Use consecutiveDrivingSeconds (resets after 30-min break) NOT total drivingSeconds
  // This is what the dashboard countdown measures — time until mandatory break is due
  const remainingDrivingSeconds = Math.max(0, drivingLimitSeconds - consecutiveDrivingSeconds);
          ...
          <View style={{ backgroundColor: "rgba(255,255,255,0.05)", borderRadius: 20, padding: 20, alignItems: "center", borderWidth: 1, borderColor: "rgba(255,255,255,0.08)" }}>
            ...
            {isShiftActive ? (
              isOnBreak ? (
                ...
                // DRIVING: normal hours countdown
                <CountdownRing
