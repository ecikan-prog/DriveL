...
/**
 * Sends an email through Resend's HTTP API. Requires RESEND_API_KEY to be
 * set in the environment. RESEND_FROM_EMAIL is optional and defaults to
 * Resend's shared sandbox sender, which works without any domain
 * verification — swap it for an @drivelegal.app address once that domain
...
async function sendResendEmail(options: {
  ...
  const from =
    process.env.RESEND_FROM_EMAIL || "Drive Legal <onboarding@resend.dev>";
