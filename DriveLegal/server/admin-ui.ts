...
// Operators are treated as active unless explicitly marked inactive.
// This keeps existing/legacy rows (isActive NULL, before the column
// existed or was backfilled) showing as active rather than locked out.
function operatorIsActive(value: unknown): boolean {
...
const adminStyles = `
  ...
  * {
  ...
  .inline-create-form .field-group {
    flex: 1 1 220px;
  }

  .inline-create-form .field-label {
    margin-bottom: 6px;
    font-size: 11px;
    font-weight: 800;
  ...
  .inline-create-form .field-input {
    padding: 10px 12px;
    border-radius: 9px;
    border: 1px solid var(--border);
  ...
  .inline-create-form .field-input:focus {
    outline: none;
    border-color: var(--primary);
    box-shadow: 0 0 0 3px rgba(93,127,231,0.22);
  }
...
export function registerAdminUi(app: Express) {
          ...
          <div class="panel-header">
            <div>
              ...
              <div class="panel-subtitle">
                Register a new operator company
              </div>
            </div>
          </div>
          ...
          >
            ...
            <div class="field-group">
              ...
              <input
                ...
                type="text"
                required
                placeholder="e.g. Queenstown Shuttles Ltd"
              />
            </div>
            ...
            <div class="field-group">
              <label class="field-label" for="temporaryPassword">
                Temporary password
              </label>
            ...
            <button class="button button-success" type="submit" style="min-height:47px;">
              Create Operator
            </button>
          </form>
        </section>
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      ...
      if (!companyName || !contactName || !email || !temporaryPassword) {
        return redirectWithMessage(
          res,
          "/admin/operators",
          "Company name, contact name, email and temporary password are required.",
          "warning"
        );
      }

      // Basic email shape check — this is an admin tool, not public
      // registration, so we keep validation light but not absent.
      ...
      if (!emailPattern.test(email)) {
        return redirectWithMessage(
          res,
          "/admin/operators",
          "Enter a valid email address for the operator.",
          "warning"
        );
      }
      ...
      // NOTE: passwordHash is a bare varchar(64) with no accompanying salt
      // column in the current schema, so this stores an unsalted SHA-256
      // hex digest to match that constraint. This is acceptable for an
      // admin-issued temporary password that the operator is expected to
      // change on first login, but it is not a strong long-term password
      // storage scheme. If operators will authenticate with this hash
      // long-term, consider widening the column and switching to a salted
      // scheme (e.g. scrypt) instead.
      ...
      try {
        ...
        await query(
          `
            ...
            VALUES (?, ?, ?, ?, NOW(), NOW())
          `,
          [email, passwordHash, companyName, contactName]
        );

        const createdRows = await query<{ id: number }>(
          `
            SELECT id
            FROM operators
            WHERE email = ?
        ...
        return redirectWithMessage(
          res,
          newId
            ? `/admin/operator/${encodeURIComponent(String(newId))}`
            : "/admin/operators",
        ...
        return redirectWithMessage(
          res,
          "/admin/operators",
          `Could not create the operator: ${detail}`,
          "error"
        );
      }
    }
        ...
        <section class="panel">
          ...
          <div class="profile-grid">
            ...
            <div class="profile-item">
              ...
              <strong>
                ${
                  operatorIsActive(operator.isActive)
            ...
            ${
              operatorIsActive(operator.isActive)
                ? `
                  <form
                    method="POST"
                    action="/admin/operator/${encodeURIComponent(String(operator.id))}/deactivate"
                    onsubmit="return confirm('Deactivate ${escapeHtml(operator.companyName)}? This disables their portal login immediately. All drivers, shifts and historical records are kept and nothing is deleted.');"
                  >
                    <input type="hidden" name="csrfToken" value="${escapeHtml(csrfToken)}" />
                    <button class="button button-warning" type="submit">Deactivate Operator</button>
                  </form>
                `
                : `
                  <form
                    method="POST"
                    action="/admin/operator/${encodeURIComponent(String(operator.id))}/reactivate"
                    onsubmit="return confirm('Reactivate ${escapeHtml(operator.companyName)}? This restores their portal login.');"
                  >
                    <input type="hidden" name="csrfToken" value="${escapeHtml(csrfToken)}" />
                    <button class="button button-warning" type="submit">Reactivate Operator</button>
                  </form>
                `
            }
          </div>
        ...
        <section class="panel">
          <div class="panel-header">
            <div>
              Danger Zone
              <div class="panel-subtitle">
                Permanent, unrecoverable action
              </div>
            </div>
          </div>

          <div class="danger-zone" style="padding: 20px;">
            ${
              linkedDrivers.length > 0
                ? `
                  <div class="empty" style="text-align:right; padding: 0;">
                    <button class="button button-danger" type="button" disabled title="Unlink all drivers before deleting this operator">
                      Delete Operator
                    </button>
                    <p class="page-note" style="margin-top:10px;">
                      This operator has ${linkedDrivers.length} linked driver${linkedDrivers.length === 1 ? "" : "s"}.
                      Unlink every driver before the account can be permanently deleted.
                      In most cases, Deactivate Operator above is the safer choice —
                      it keeps drivers, shifts and historical records intact.
                    </p>
                  </div>
                `
                : `
                  <form
                    method="POST"
                    action="/admin/operator/${encodeURIComponent(String(operator.id))}/delete"
                    onsubmit="return confirm('Permanently delete ${escapeHtml(operator.companyName)}? This cannot be undone. Consider Deactivate Operator instead if you may need this account again.');"
                  >
                    <input type="hidden" name="csrfToken" value="${escapeHtml(csrfToken)}" />
                    <button class="button button-danger" type="submit">Delete Operator</button>
                  </form>
                `
            }
          </div>
  ...
  /* OPERATOR — DEACTIVATE / REACTIVATE / DELETE */

  app.post(
    "/admin/operator/:id/deactivate",
    async (req: Request, res: Response) => {
      ...
      try {
        const rows = await query<{ id: number; companyName: string }>(
          `SELECT id, companyName FROM operators WHERE id = ? LIMIT 1`,
          [operatorId]
        );
        ...
        // Deactivating only flips a flag and stamps when it happened.
        // Drivers, operator_drivers links and shift_logs are never
        // touched, so all historical/compliance records are preserved
        // and the operator can be reactivated later with everything
        // intact.
        await query(
          `
            UPDATE operators
        ...
        return redirectWithMessage(
          res,
          `/admin/operator/${encodeURIComponent(String(operatorId))}`,
          `${operator.companyName} has been deactivated. Their portal login is now disabled; all drivers and shift history are unaffected.`,
          "warning"
        );
      } catch (error) {
        console.error("[ADMIN DEACTIVATE OPERATOR ERROR]", error);
        return res.status(500).send("Could not deactivate operator");
      }
    }
  );

  app.post(
    "/admin/operator/:id/reactivate",
    async (req: Request, res: Response) => {
      if (!requireAdmin(req, res) || rejectInvalidCsrf(req, res)) {
        return;
      }
      ...
      try {
        ...
        return redirectWithMessage(
          res,
          `/admin/operator/${encodeURIComponent(String(operatorId))}`,
          `${operator.companyName} has been reactivated.`
        );
      } catch (error) {
        console.error("[ADMIN REACTIVATE OPERATOR ERROR]", error);
        return res.status(500).send("Could not reactivate operator");
      }
    }
  );

  app.post(
    "/admin/operator/:id/delete",
    async (req: Request, res: Response) => {
      if (!requireAdmin(req, res) || rejectInvalidCsrf(req, res)) {
        return;
      }
      ...
      try {
        ...
        // Safety check performed here in addition to whatever the schema
        // enforces: an operator with any linked drivers can never be
        // deleted. This keeps driver/shift history from ever being
        // orphaned or silently cascaded away.
        const linkedRows = await query<{ count: number | string }>(
          `
            SELECT COUNT(*) AS count
            FROM operator_drivers
            WHERE operatorId = ?
        ...
        if (linkedCount > 0) {
          return redirectWithMessage(
            res,
            `/admin/operator/${encodeURIComponent(String(operatorId))}`,
            `Could not delete ${operator.companyName}: it still has ${linkedCount} linked driver${linkedCount === 1 ? "" : "s"}. Unlink every driver first, or use Deactivate instead.`,
            "error"
          );
        }

        await query(`DELETE FROM operators WHERE id = ?`, [operatorId]);

        return redirectWithMessage(
          res,
          "/admin/operators",
          `${operator.companyName} was permanently deleted.`
        );
      } catch (error) {
        console.error("[ADMIN DELETE OPERATOR ERROR]", error);

        // If the database itself has a foreign key constraint guarding
        // operator_drivers -> operators, a deletion attempt with linked
        // drivers will throw here even if the check above is ever
        // bypassed. Surface that as the same clear, safe error rather
        // than a raw 500.
        return redirectWithMessage(
          res,
          `/admin/operator/${encodeURIComponent(String(operatorId))}`,
          "Could not delete this operator. It may still have linked drivers or other related records — use Deactivate instead.",
          "error"
        );
      }
    }
